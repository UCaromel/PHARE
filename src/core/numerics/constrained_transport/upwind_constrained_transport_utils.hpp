...
class UpwindConstrainedTransportState
{
    ...
    // jt/rhot exist iff the scheme carries a current, and that is the predicate GodunovFluxes
    // uses to pick the save() overload which writes them (godunov_fluxes.hpp, the
    // `Hall || Resistivity || HyperResistivity` branch). They are read back by the Hall EMF
    // terms in UpwindConstrainedTransport, by the resistive flux contribution, and by the
    // spatial hyper-resistive term via getJt/getRhot. Anything narrower than the writer's own
    // predicate leaves save() writing through an unset buffer.
    constexpr static bool withTransverseCurrent = Hall || Resistivity || HyperResistivity;
...
public:
    ...
    NO_DISCARD auto getCompileTimeResourcesViewList()
    {
        ...
        {
            if constexpr (withTransverseCurrent)
