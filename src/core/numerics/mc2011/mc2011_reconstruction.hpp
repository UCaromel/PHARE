...
//
// Frozen literals are exact-rational (SymPy) outputs evaluated to double,
// locked in-tree by tests/amr/solvers/time_integrator/test_mc2011_kernels.cpp.
//
// References:
...
struct SSPRK54
{
    ...
    static constexpr double b[5]{w0 * w11 * w21 * w31 * w43 + w0 * w11 * w21 * w41 + w0 * w11 * w40,
                                 w12 * w21 * w31* w43 + w12 * w21 * w41 + w12* w40,
                                 w22 * w31 * w43 + w22* w41, w32* w43 + w42, w44};
    ...
    // endpoint (chi=1) reproduces the SSPRK update bit-for-bit.
    static constexpr double beta3[5]{b[0] - beta1[0] - beta2[0], b[1] - beta1[1] - beta2[1],
                                     b[2] - beta1[2] - beta2[2], b[3] - beta1[3] - beta2[3],
                                     b[4] - beta1[4] - beta2[4]};

    // Per-stage geometric constants: gamma1=c_i, gamma2=(Ac)_i,
    // gamma3=1/2(Ac^2)_i, gamma4=(AAc)_i. Index 0..3 = the four intermediate
    // states (Butcher stages Y2..Y5); index 4 = the final blended state,
    // whose entries are the exact order-condition values {1, 1/2, 1/6, 1/6}.
    // Stage rows are constexpr-derived from the w's by the Shu-Osher
    // convexity recursion, same pattern as c[].
...
// the persisted post-stage states of one coarse step: y0 = u_n, y1..y4 = the
// Shu-Osher stage states Y2..Y5, unp1 = the final blended state, dtC = the
// sweep's own dt.
//
//   k1 = (y1 - y0)/(w0*dtC)
...
// bit-identical to the k's the (Unew - u0)/dt capture used to produce. Row 5
// is the Butcher-residual variant: unconditionally exact under any
// re-literalization of the tableau, with the chi=1 endpoint bit-exact against
// the stored unp1, and it reuses the production accumulation weights b[]
// verbatim.
                                          ...
                                          double const dtC)
{
    ...
    double const acc
        = SSPRK54::b[0] * k1 + SSPRK54::b[1] * k2 + SSPRK54::b[2] * k3 + SSPRK54::b[3] * k4;
...
// derivatives k_i = f(Y_i) of one coarse step.
...
// fraction chi in [0,1] of the coarse step:
//
                             ...
                             double const dtFine, std::size_t const stageIndex)
{
    ...
    {
        double const b_i = SSPRK54::beta1[s] * chi + SSPRK54::beta2[s] * chi * chi
                           + SSPRK54::beta3[s] * chi * chi * chi;
        double const bp_i = SSPRK54::beta1[s] + 2.0 * SSPRK54::beta2[s] * chi
                            + 3.0 * SSPRK54::beta3[s] * chi * chi;
