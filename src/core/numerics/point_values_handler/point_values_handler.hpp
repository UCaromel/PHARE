...
public:
    ...
    void point_value_fluxes_to_integral(auto& fluxes, auto& E) const
    {
        ...
        auto convert_hydro_fluxes = [&]<auto dir_tag>() {
            ...
            convert(fluxes.rho_fx);
            convert(fluxes.Etot_fx);
            for (int i = 0; i < 3; ++i)
                convert(fluxes.rhoV_fx(static_cast<core::Component>(i)));
        };
...
private:
    ...
    void cell_center_conversion_(Field_t const& f, Field_t& fnew, MeshIndex<dimension> index)
    {
        fnew(index) = get_cell_center_<mode>(f, index);
    }

    template<ConversionMode mode>
    void cell_center_conversion_(Field_t const& f, Field_t& fnew, MeshIndex<dimension> index) const
    {
        fnew(index) = get_cell_center_<mode>(f, index);
    }
    ...
    void face_center_conversion_(Field_t const& f, Field_t& fnew, MeshIndex<dimension> index)
    {
        fnew(index) = get_face_center_<direction, mode>(f, index);
    }

    template<auto direction, ConversionMode mode>
    void face_center_conversion_(Field_t const& f, Field_t& fnew, MeshIndex<dimension> index) const
    {
        fnew(index) = get_face_center_<direction, mode>(f, index);
    }
    ...
    template<auto direction, ConversionMode mode>
    void edge_center_conversion_(Field_t const& f, Field_t& fnew, MeshIndex<dimension> index)
    {
        fnew(index) = get_edge_center_<direction, mode>(f, index);
    }

    template<auto direction, ConversionMode mode>
    void edge_center_conversion_(Field_t const& f, Field_t& fnew, MeshIndex<dimension> index) const
    {
        fnew(index) = get_edge_center_<direction, mode>(f, index);
    }
