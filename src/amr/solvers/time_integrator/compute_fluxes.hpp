...
class ComputeFluxes
{
    ...
    using NonIdealEMF_t = Dispatchers_t::NonIdealEMF_t;
...
public:
                    ...
                    double const newTime)
    {
        ...
        if (isNonIdeal_)
            NonIdealEMF_t{level, model, fVMethodInfo_}(emf_, state);

        FVMethod_t{level, model, fVMethodInfo_}(ct_, emf_, state, fluxes, newTime);
        ...
        ConstrainedTransport_t{level, model, constrainedTransportInfo_}(ct_, emf_, state);
    }

    void registerResources(MHDModel& model)
    {
        model.resourcesManager->registerResources(ct_);
        model.resourcesManager->registerResources(emf_);
    }

    void allocate(MHDModel& model, auto& patch, double const allocateTime) const
    {
        model.resourcesManager->allocate(ct_, patch, allocateTime);
        model.resourcesManager->allocate(emf_, patch, allocateTime);
    }

private:
    FVMethodInfo_t fVMethodInfo_;
    ConstrainedTransportInfo_t constrainedTransportInfo_;
    bool const isNonIdeal_{fVMethodInfo_.isResistive() || fVMethodInfo_.isHyperResistive()};

    core::UpwindConstrainedTransportState<VecField> ct_{Hall};
    core::NonIdealEMFState<VecField> emf_{isNonIdeal_};
