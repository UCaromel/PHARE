...
template<typename GridLayoutT, typename Grid_t, typename PhysicalQuantity>
void FieldData<GridLayoutT, Grid_t, PhysicalQuantity>::unpackStreamAndMax(
    SAMRAI::tbox::MessageStream& stream, SAMRAI::hier::BoxOverlap const& overlap)
{
    using SetMaxOp = core::SetMax<value_type>;

    unpackStream<SetMaxOp>(stream, overlap, field);
}

template<typename GridLayoutT, typename Grid_t, typename PhysicalQuantity>
void FieldData<GridLayoutT, Grid_t, PhysicalQuantity>::max(SAMRAI::hier::PatchData const& src,
                                                           SAMRAI::hier::BoxOverlap const& overlap)
{
    using SetMaxOp = core::SetMax<value_type>;
    ...
    copy_<SetMaxOp>(fieldSource, fieldOverlap, field);
}
