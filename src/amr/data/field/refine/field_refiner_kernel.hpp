 ...
 *
 * Fills ALL fine faces per component (shared and interior) with the order-dialed tensor
 * stencils; the stage-2 partner ADPTMagneticRefinePatchStrategy::postprocessRefine then applies
 * the order-independent divB touch-up. The shared-face tangential correction is antisymmetric
 * in the child sign, so ∇·B is preserved at every order, limited or not.
 */
template<typename GridLayoutT, typename FieldT, Representation R = Representation::Average>
std::shared_ptr<IFieldRefineKernel<GridLayoutT, FieldT>>
