...
template<typename GridLayoutT, typename FieldT, std::size_t order, typename Limiter = NoLimiter,
         Representation representation = Representation::Average>
...
makeRefineKernel(int order, std::string const& limiter)
{
    auto build = [&limiter]<std::size_t O>() -> std::shared_ptr<IFieldRefineKernel<GridLayoutT, FieldT>> {
        if (limiter == "none" || limiter.empty())
            return std::make_shared<CompositeFieldRefiner<GridLayoutT, FieldT, O, NoLimiter, R>>();
        if (limiter == "minmod")
            return std::make_shared<
                CompositeFieldRefiner<GridLayoutT, FieldT, O, core::MinModLimiter, R>>();
        if (limiter == "vanleer")
            return std::make_shared<
                CompositeFieldRefiner<GridLayoutT, FieldT, O, core::VanLeerLimiter, R>>();
