 ...
 * 4th-order div-free B refinement.
 *
 * Derivation: ~/.claude/plans/ucaromel/higher-order-refinement/2026-07-12-balsara-divfree-prolongation/
 *             derivation.md (§S4 2D, §S5 3D, §S10 implementation entry);
 *             anisotropic generalisation: ../2026-07-17-adpt-anisotropic-touchup/derivation.md
 *             (§S11–§S15, §S15 = operator spec).
 *
 * IMPLEMENTATION NOTES
 * --------------------
 * - Anisotropic meshes (dx != dy != dz) are handled exactly (derivation §S11–§S15, flux gauge,
 *   Eq. 41): subzoneDiv weights each face difference by 1/D_c and the correction of a component-c
 *   face carries a D_c prefactor. The integer stencils [3,1]/8 (2D) and [7,2,2,1]/24 (3D) survive
 *   anisotropy verbatim — only the divergence proxy and the prefactor change. Only the anisotropy
 *   RATIO matters: a uniform rescale of all D_c cancels between the weighted divergence and the
 *   prefactor, so equal-mesh reduction to the historical unweighted operator is an identity and
 *   fine-vs-coarse meshSize is immaterial. 1D is unchanged (weights cancel, §S11 check 6).
 ...
 *   uniform scale). The closed-form denominators stay the FLUX-variable ones: 1/2 (1D), [3,1]/8
 *   (2D), [7,2,2,1]/24 (3D) — the density weights of §S4/§S5 ([·]/16, [·]/48) rescaled by the
 *   fixed refinement ratio 2; with the D_c prefactor this numerically reproduces TR exactly at
 *   equal mesh.
...
public:
    ...
    {
        assert(b_id_ >= 0
               && "ADPTMagneticRefinePatchStrategy: IDs must be registered before use");
    }

    void registerIDs(int const b_id) override { b_id_ = b_id; }


    // Stage-2 touch-up: same postprocessRefine slot / loop shape as the legacy TR strategy
    // (fine_box → per-component field box via FieldGeometry::toFieldBox, per-dimension dispatched
    // loops, isNewFineFace odd-parity gate, AMRToLocal indexing, B-triple via getFields).
                            ...
                            core::Point<int, dimension> idx)
    {
        ...
        bx(ix, iy, iz) += D[dirX]
                          * (7.0 * pair(iy, iz) + 2.0 * pair(sy, iz) + 2.0 * pair(iy, sz)
                             + pair(sy, sz))
                          / 24.0;
    }
                            ...
                            core::Point<int, dimension> idx)
    {
        ...
        by(ix, iy, iz) += D[dirY]
                          * (7.0 * pair(ix, iz) + 2.0 * pair(sx, iz) + 2.0 * pair(ix, sz)
                             + pair(sx, sz))
                          / 24.0;
    }
                            ...
                            core::Point<int, dimension> idx)
    {
        ...
        bz(ix, iy, iz) += D[dirZ]
                          * (7.0 * pair(ix, iy) + 2.0 * pair(sx, iy) + 2.0 * pair(ix, sy)
                             + pair(sx, sy))
                          / 24.0;
    }
...
private:
    ...
    // directions of (high face − low face)/D_c (§S15 dw; 1D keeps the raw difference — the weight
    // cancels against the prefactor). Memoised so every interior face's correction reads the
    // STAGE-1 value even after sibling faces are written.
    ...
    static double subzoneDiv2d_(auto& cache, auto& bx, auto& by, auto const& D, int cx, int cy)
    {
        ...
        if (auto it = cache.find(key); it != cache.end())
            return it->second;
        double const d = (bx(cx + 1, cy) - bx(cx, cy)) / D[dirX]
