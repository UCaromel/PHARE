...
struct SimOpts
{
    ...
    {
        bool const off = !mhd_enabled;
        return off == (time_integrator_type == MHDOpts::TimeIntegratorType::MHDOff)
               && off == (slope_limiter_type == MHDOpts::SlopeLimiterType::MHDOff)
               && off == (riemann_solver_type == MHDOpts::RiemannSolverType::MHDOff);
    }
};

template<SimOpts opts>
inline constexpr bool is_hybrid_v = opts.hybrid_enabled;
