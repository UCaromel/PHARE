...
class WhistlerConvergenceTest(ConvergenceTestBase):
    ...
    # Per-order bands since the anisotropic ADPT touch-up (see the alfven test
    # for the rationale; the old shared cap at 2 was the touch-up's unweighted
    # divergence proxy on this dy/dx != 1 grid). Measured kaa 2026-07-18
    # (30 ranks): order=4 slope 4.01 (segments 4.02/4.00) -- matching the iso
    # discriminators; order=2 slope 3.38 (3.51/3.25), segment-drifting mixed
    # regime, loose band whose lower edge just beats the old 2.25 cap.
