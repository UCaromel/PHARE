/*
  ...
  The k-capture overloads (FiniteVolumeEulerPerField, Faraday) are retired from
  production -- the messenger back-solves the stage derivatives from the
  persisted stage states (core::mc2011::backSolve) instead. This file
  reconstructs oracle values test-side from the plain production API (no
  capture parameter reintroduced), via two DIFFERENT, independently-computed
  routes for gate G1:
    - a bit-identical pinned-formula reconstruction, (Y_i - combo)/(w_i*dt),
      which mirrors backSolve's own documented algebra operand-for-operand
      (StateBackSolve.rowsOneToFourBitIdenticalToRetiredCapture): this pins
      backSolve's current implementation but, because it is the same formula
      typed twice, cannot by itself catch a bug that is consistent between
      backSolve() and this reconstruction (e.g. a typo copied into both);
    - a direct per-stage flux-divergence oracle, using the dt=1,U=0
      raw-divF trick (same technique row 5 already used) on each stage's OWN
      flux pair Fi{x,y} rather than inverting the stage state Y_i
      (StateBackSolve.rowsOneToFourMatchDirectFluxDivergence): this is
...
// A coherent pure-MHD profile purely to name the 2D Yee-MHD GridLayout_t type below.
// {2, 1} (dimension, interp_order) is a *Hybrid* profile (reconstruction_type stays
// MHDOff), so PHARE_Types<...>::MHD is an incoherent-MHD-axes type there; naming
// MHD::GridLayout_t off it hits the deliberate absent-MHDOff-specialization canary
// (ReconstructionNghosts<MHDOff> is intentionally never defined). interp_order=0 keeps
// this profile pure MHD; the reconstruction/timestepper/order values below are
// otherwise arbitrary valid choices (these tests build GridLayout directly from
// dl/cells/origin, not from this profile).
constexpr PHARE::SimOpts kMHD2DOpts{2,
                                    0,
                                    0,
...
{
    using Layout = typename PHARE::core::PHARE_Types<kMHD2DOpts>::MHD::GridLayout_t;
    ...
    UsableFieldMHD<2> zero{"zero", layout, MHDQuantity::Scalar::rho};
    UsableFieldMHD<2> kFlux{"kFlux", layout, MHDQuantity::Scalar::rho};
    ...
    layout.evalOnGhostBox(Fx,
                          [&](auto const&... args) { Fx(args...) = fval(0.2, 0.7, 0.5, args...); });
    layout.evalOnGhostBox(
        Fy, [&](auto const&... args) { Fy(args...) = fval(-0.1, 0.6, 0.8, args...); });

    double const dt = 0.01;
    FiniteVolumeEulerPerField<Layout> const fvEuler{layout, dt};
    // dt=1, U=0 is the production formula's own way to expose the raw flux
    // divergence k=-divF (dt-independent) with no capture-output parameter:
    // FvE always computes Unew = U - dt*divF, so this reduces to Unew = -divF.
    FiniteVolumeEulerPerField<Layout> const fvEulerRawDivF{layout, 1.0};

    fvEuler(U, Unew, Fx, Fy);            // distinct output (stage-1 pattern)
    fvEuler(Ualias, Ualias, Fx, Fy);     // in-place (stages 2-5 pattern), same inputs as above
    fvEulerRawDivF(zero, kFlux, Fx, Fy); // k = -divF, independent of the alias pattern above

    double maxAbsK = 0.0;
    layout.evalOnBox(Unew, [&](auto const&... args) {
        EXPECT_DOUBLE_EQ(Ualias(args...), Unew(args...));
        maxAbsK = std::max(maxAbsK, std::abs(kFlux(args...)));
    });
...
TEST(KCapture, faradayAliasedMatchesDistinct)
{
    ...
    for (auto const component : {Component::X, Component::Y, Component::Z})
    {
        ...
        layout.evalOnGhostBox(
            Ec, [&](auto const&... args) { Ec(args...) = fval(0.1, 0.45, 0.55, args...); });
    }

    double const dt = 0.01;
    Faraday<Layout> faraday{layout};

    // Faraday no longer exposes a k-capture output (production dropped that overload;
    // the messenger back-solves stage derivatives from persisted stage states instead,
    // same as FiniteVolumeEulerPerField above). Alias coverage still stands on Faraday's
    // one production overload: B and Balias hold identical inputs against the same E, so
    // a distinct-output call (stage-1 pattern) and an in-place aliased call (stages 2-5
    // pattern) must agree. "k" here is derived locally as (Bnew-B)/dt purely to guard
    // against the degenerate always-equal-to-input case (the original F1 failure mode),
    // not to exercise any production capture path.
    faraday(B, E, Bnew, dt);        // distinct output (stage-1 pattern)
    faraday(Balias, E, Balias, dt); // in-place (stages 2-5 pattern), same inputs as above

    // Bx is untouched in 1D only; in 2D all three components evolve.
    for (auto const component : {Component::X, Component::Y, Component::Z})
    {
        double maxAbsK = 0.0;
        layout.evalOnBox(Bnew(component), [&](auto const&... args) {
            EXPECT_DOUBLE_EQ(Balias(component)(args...), Bnew(component)(args...));
            double const k = (Bnew(component)(args...) - B(component)(args...)) / dt;
            maxAbsK        = std::max(maxAbsK, std::abs(k));
        });
...
TEST(SSPRK54Coefficients, betaSumsReproduceButcherWeights)
{
    ...
    for (int i = 0; i < 5; ++i)
        EXPECT_NEAR(SSPRK54::beta1[i] + SSPRK54::beta2[i] + SSPRK54::beta3[i], SSPRK54::b[i], 1e-15)
            << "i = " << i;
}
...
protected:
    ...
    UsableFieldMHD<2> zero{"zero", layout, MHDQuantity::Scalar::rho}; // never written; stays 0
    ...
    // Independent oracle for rows 1-4 (StateBackSolve.rowsOneToFourMatchDirectFluxDivergence):
    // -divF_i evaluated directly from stage i's own flux pair, structurally independent of
    // backSolve's (Y_i - combo)/(w_i*dt) inversion algebra.
    UsableFieldMHD<2> k1Direct{"k1Direct", layout, MHDQuantity::Scalar::rho};
    UsableFieldMHD<2> k2Direct{"k2Direct", layout, MHDQuantity::Scalar::rho};
    UsableFieldMHD<2> k3Direct{"k3Direct", layout, MHDQuantity::Scalar::rho};
    UsableFieldMHD<2> k4Direct{"k4Direct", layout, MHDQuantity::Scalar::rho};
    ...
    void SetUp() override
    {
        ...
        auto fillFlux = [&](auto& F, double const a, double const fx, double const fy) {
            layout.evalOnGhostBox(
                F, [&](auto const&... args) { F(args...) = fval(a, fx, fy, args...); });
        };
        ...
        auto comb = [&](auto& out, double const wa, auto const& a, double const wb, auto const& b) {
        ...
        FvE{layout, SSPRK54::w0 * dt}(U0, Y1, F1x, F1y);

        comb(Y2, SSPRK54::w10, U0, SSPRK54::w11, Y1);
        FvE{layout, SSPRK54::w12 * dt}(Y2, Y2, F2x, F2y);

        comb(Y3, SSPRK54::w20, U0, SSPRK54::w21, Y2);
        FvE{layout, SSPRK54::w22 * dt}(Y3, Y3, F3x, F3y);

        comb(Y4, SSPRK54::w30, U0, SSPRK54::w31, Y3);
        FvE{layout, SSPRK54::w32 * dt}(Y4, Y4, F4x, F4y);
        ...
        // Retired-capture oracle, reconstructed test-side from the plain production
        // API only (no capture parameter added to FiniteVolumeEulerPerField):
        // rows 1-4 mirror backSolve's own documented reconstruction formula exactly
        // -- (Y_i - combo)/(w_i*dt), same weights, same operand order -- so gate G1's
        // EXPECT_EQ is bit identity by construction, not an independently-rounded path.
        layout.evalOnBox(U0, [&](auto const&... args) {
            k1cap(args...) = (Y1(args...) - U0(args...)) / (SSPRK54::w0 * dt);
            k2cap(args...)
                = (Y2(args...) - (SSPRK54::w10 * U0(args...) + SSPRK54::w11 * Y1(args...)))
                  / (SSPRK54::w12 * dt);
            k3cap(args...)
                = (Y3(args...) - (SSPRK54::w20 * U0(args...) + SSPRK54::w21 * Y2(args...)))
                  / (SSPRK54::w22 * dt);
            k4cap(args...)
                = (Y4(args...) - (SSPRK54::w30 * U0(args...) + SSPRK54::w31 * Y3(args...)))
                  / (SSPRK54::w32 * dt);
        });

        // Row 5's retired capture computed -divF(F5x,F5y) directly (dt-independent):
        // dt=1, U=zero is FiniteVolumeEulerPerField's own way to expose that raw
        // divergence with no capture-output parameter (Unew = 0 - 1*divF = -divF).
        FvE{layout, 1.0}(zero, k5cap, F5x, F5y);

        // Independent oracle for rows 1-4: the same dt=1,U=zero raw-divergence trick
        // as row 5 above, applied to each stage's OWN flux pair (Fix,Fiy) instead of
        // inverting Y_i. This never reads Y1..Y4/U0 and never touches backSolve's
        // (Y_i - combo)/(w_i*dt) formula, so it cannot share a transcription bug with
        // either backSolve() or the k1cap..k4cap reconstruction above.
        FvE{layout, 1.0}(zero, k1Direct, F1x, F1y);
        FvE{layout, 1.0}(zero, k2Direct, F2x, F2y);
        FvE{layout, 1.0}(zero, k3Direct, F3x, F3y);
        FvE{layout, 1.0}(zero, k4Direct, F4x, F4y);
    }
};


TEST_F(StateBackSolve, rowsOneToFourBitIdenticalToRetiredCapture) // gate G1 (pinned formula)
{
    ...
    //
    // NOTE: k1cap..k4cap are computed test-side with the *same* (Y_i - combo)/(w_i*dt)
    // formula backSolve() itself implements (see their definition in SetUp() above), so
    // this gate pins backSolve's current implementation against its own documented
    // algebra -- it cannot, by itself, catch a wrong weight/operand/sign that ends up
    // identical in both places. StateBackSolve.rowsOneToFourMatchDirectFluxDivergence
    // below is the structurally-independent oracle for rows 1-4; both gates are kept.
    double maxAbsK = 0.0;
    layout.evalOnBox(U0, [&](auto const&... args) {
...
TEST_F(StateBackSolve, rowsOneToFourMatchDirectFluxDivergence) // gate G1 (independent oracle)
{
    // Independent check for rows 1-4: k1Direct..k4Direct (SetUp() above) are -divF_i
    // evaluated directly from each stage's own flux pair via FiniteVolumeEulerPerField's
    // dt=1,U=zero raw-divergence trick -- the same technique row 5 already used below --
    // rather than inverting Y_i with backSolve's own (Y_i - combo)/(w_i*dt) formula. In
    // exact arithmetic the two are equal, since Y_i was itself built as
    // combo_i - (w_i*dt)*divF_i, so (Y_i - combo_i)/(w_i*dt) == -divF_i algebraically.
    //
    // They are not bit-identical in floating point: forming Y_i multiplies divF_i by the
    // small coefficient w_i*dt (~4e-3 to ~5.5e-3 here, dt=0.01) and adds it to combo_i
    // (O(1) in magnitude), then backSolve's inversion subtracts combo_i back off and
    // divides by that same small w_i*dt -- a subtract-small-from-large-then-divide
    // pattern that amplifies the O(eps) rounding of the Y_i-combo_i subtraction by
    // 1/(w_i*dt) ~ O(1e2-1e3). This is the same order of cancellation amplification
    // already used to justify the 1e-10*scale tolerance on row 5 below (there the
    // amplification is 1/(b5*dt) ~ 4e2), so the same bound is reused here rather than
    // inventing a separate, weaker one.
    double maxAbsK[4] = {0.0, 0.0, 0.0, 0.0};
    layout.evalOnBox(U0, [&](auto const&... args) {
        auto const k = mc2011::backSolve(U0(args...), Y1(args...), Y2(args...), Y3(args...),
                                         Y4(args...), UNP1(args...), dt);
        double const direct[4]
            = {k1Direct(args...), k2Direct(args...), k3Direct(args...), k4Direct(args...)};
        for (std::size_t i = 0; i < 4; ++i)
        {
            double const scale = std::max({1.0, std::abs(direct[i]), std::abs(k[i])});
            EXPECT_NEAR(k[i], direct[i], 1e-10 * scale) << "row " << (i + 1);
            maxAbsK[i] = std::max(maxAbsK[i], std::abs(direct[i]));
        }
    });
    // Each stage individually, not just their aggregate max: a bug that zeroes exactly
    // one row (e.g. a wrong flux pair wired to one FvE call) must not hide behind the
    // other three rows being nonzero.
    EXPECT_GT(maxAbsK[0], 0.0) << "row 1";
    EXPECT_GT(maxAbsK[1], 0.0) << "row 2";
    EXPECT_GT(maxAbsK[2], 0.0) << "row 3";
    EXPECT_GT(maxAbsK[3], 0.0) << "row 4";
}


TEST_F(StateBackSolve, butcherResidualK5MatchesDirectCapture) // gate G1, row 5
{
    // k5-hat is the butcher residual; the retired capture computed -divF(F5x,F5y)
