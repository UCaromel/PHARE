...
using ImplYee1 = PHARE::core::hybrid::GridLayoutImplYee<
    PHARE::HybridFieldOptions<PHARE::SimOpts{1, 1}>{}>;
...
TEST(DualProlongation, order4WeightsMatchLadder)
{
    auto right = ImplYee1::directionalProlongation<dirX, +1, 4>(); // σ=+1
    EXPECT_DOUBLE_EQ(coefAt(right, -2), 3. / 128.);
    EXPECT_DOUBLE_EQ(coefAt(right, -1), -22. / 128.);
    EXPECT_DOUBLE_EQ(coefAt(right, 0), 1.);
    EXPECT_DOUBLE_EQ(coefAt(right, +1), 22. / 128.);
    EXPECT_DOUBLE_EQ(coefAt(right, +2), -3. / 128.);
}
...
TEST(DualProlongation, weightsSumToOne)
{
    ...
    EXPECT_DOUBLE_EQ(coefSum(ImplYee1::directionalProlongation<dirX, -1, 2>()), 1.);
    EXPECT_DOUBLE_EQ(coefSum(ImplYee1::directionalProlongation<dirX, +1, 4>()), 1.);
    EXPECT_DOUBLE_EQ(coefSum(ImplYee1::directionalProlongation<dirX, -1, 4>()), 1.);
}
...
TEST(DualProlongation, childrenMeanBackToCoarse)
{
    ...
    check(ImplYee1::directionalProlongation<dirX, +1, 4>(),
          ImplYee1::directionalProlongation<dirX, -1, 4>());
}
...
TEST(DualProlongation, exactOnLinearData)
{
    ...
    // order 4 must also be exact on linear data
    for (auto const& [row, sigma] :
         {std::pair{ImplYee1::directionalProlongation<dirX, +1, 4>(), +1},
          std::pair{ImplYee1::directionalProlongation<dirX, -1, 4>(), -1}})
        EXPECT_DOUBLE_EQ(applyRow(row, I, f), a + b * (I + sigma * 0.25));
}
...
{
    using ImplYee2
        = PHARE::core::hybrid::GridLayoutImplYee<PHARE::HybridFieldOptions<PHARE::SimOpts{2, 1}>{}>;
