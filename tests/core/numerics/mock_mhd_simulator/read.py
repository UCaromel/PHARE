...
def read_and_plot_h5(file_path, timestep, quantity):
    ...
    with h5py.File(file_path, "r") as h5_file:
        ...
        if quantity not in h5_file[timestep]:
            raise ValueError(f"Quantity '{quantity}' not found in timestep '{timestep}'.")
        ...
        plt.imshow(data.T, origin='lower', cmap='viridis', aspect='auto')
