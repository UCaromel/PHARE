...
def config(mode, order, limiter, diag_dir):
    ...
    # refinement_order 0/2/4 routes the magnetic refiner in the MHD messenger through the
    # composite prolongation (+ ADPT divB touch-up) -- the path this test guards. Point values
    # are local derived quantities (no schedules), so only the average-world fills are exercised.
    common = dict(
        time_step=time_step,
