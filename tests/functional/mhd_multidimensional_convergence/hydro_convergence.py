...
def config(nx):
    ...
    eps = 1.e-6
    background = np.asarray([1., 1. if mode == "Entropy" else 0., 0., 0., 0., 0., 0., 1./sim.gamma])
    Rsound_p = np.asarray([1., 1., 0., 0., 0., 0., 0., 1.])
    Rsound_m = np.asarray([1., -1., 0., 0., 0., 0., 0., 1.])
    Rshear_p = np.asarray([0., 0., 1., 0., 0., 0., 0., 0.])
    Rshear_m = np.asarray([0., 0., 0., 1., 0., 0., 0., 0.])
    Rentrop = np.asarray([1., 0., 0., 0., 0., 0., 0., 0.])
    ...
    def vx(x,y,z):
        return v_bg[0] + eps * Rv[0] * phase(x,y,z)

    def vy(x,y,z):
        return v_bg[1] + eps * Rv[1] * phase(x,y,z) 

    def vz(x,y,z):
        return v_bg[2] + eps * Rv[2] * phase(x,y,z) 
    ...
    def p(x,y,z):
        return background[7] + eps * R[7] * phase(x,y,z) 
